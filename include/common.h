...
/*
x,y,z 는 PCL 평균좌표 u,v는 cam에서 좌표
direction과 direction_lidar는 각각
cam과 lidar에서의 방향벡터(고유값이 가장 높은 고유벡터)
*/
struct VPnPData
{
  double x, y, z, u, v;
...
{
  std::cout << "please input matrix element's data" << std::endl;
  for (int i = 1; i < 4; i++) {
    for (int j = 1; j < 5; j++) {
      std::cin >> matrix[i][j];
    }
...
template <class T> void calc(T matrix[4][5], Eigen::Vector3d &solution)
{
  T base_D = matrix[1][1] * matrix[2][2] * matrix[3][3] +
             ...
             matrix[3][1] * matrix[1][2] * matrix[2][3]; //행렬식 계산 싸루스
