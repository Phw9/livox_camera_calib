...
/*
첫번째 Calibration initialize
1. 이미지에서 canny edge 추출
2. 복셀 만들고 라이다에서 edge 추출
*/
...
// 라이다 PCL을 projection 시켜서 주어진 extrinsic과 이미지의 rgb를 사용해 색깔을 입혀서 color_cloud에 저장
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr &color_cloud)
{
  ...
  // 뎁스가 2~50 사이고, 라이다의 intensity가 reflectivity가 5이상일 때 pts_3d에 넣음
  ...
  // 3d에서 2d로 projection 시키기 위해 intrinsic 값이랑 extrinsic 값을 넣어줌
  ...
  // 위 파라미터들로 projection 시켰을 때 2d 포인트를 반환해줌
  ...
  {
    //projection 시켰을 때 이미지를 벗어나지 않은 애들만 볼것임
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr &lidar_line_cloud_3d)
{
  ...
  for (auto iter = voxel_map.begin(); iter != voxel_map.end(); iter++)
  {
    ...
    if (iter->second->cloud->size() > 50)
    {
      ...
      // 결과 로깅을 위한 모델 매개변수 객체 생성 == ax + bx + cy + d = 0(a,b,c,d)
      pcl::ModelCoefficients::Ptr coefficients(new pcl::ModelCoefficients);
      // inliers 포인트 클라우드 인덱스를 기록
      pcl::PointIndices::Ptr inliers(new pcl::PointIndices);
      // 스플리터 만들기
      pcl::SACSegmentation<pcl::PointXYZI> seg;
      // 선택 사항, 결과 평면에 표시되는 점이 분할 점인지 나머지 점인지 설정
      seg.setOptimizeCoefficients(true);
      // 필수 - 대상 지오메트리를 설정합니다.
      seg.setModelType(pcl::SACMODEL_PLANE);
      // 세분화 방법: 랜덤 샘플링 방법
      seg.setMethodType(pcl::SAC_RANSAC);
      // 임계값인 허용 오차 범위를 설정합니다.
